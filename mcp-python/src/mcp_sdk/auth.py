    ...
    async def get_token(self, force_refresh: bool = False, max_retries: int = 3) -> TokenData:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.info(f"Getting token (attempt {attempt + 1}/{max_retries + 1}): {auth_url}")
                ...
                async with self._session.get(auth_url, headers=headers) as response:
                    if response.status != 200:
                        ...
                        if response.status == 401:
                            try:
                                ...
                                if error_code == "10004":
                                    logger.error(f"Non-retryable error: HTTP 401 with error_code 10004")
                                    raise last_error
                                
                                # For other 401 errors, allow retry
                                if attempt < max_retries:
                                    logger.warning(f"HTTP 401 with error_code {error_code}, retrying... ({attempt + 1}/{max_retries})")
                        ...
                        if attempt < max_retries:
                            logger.warning(f"HTTP {response.status} error, retrying... ({attempt + 1}/{max_retries})")
                            await asyncio.sleep(1)  # Wait 1 second before retry
                            continue
                        raise last_error
                    ...
                    if not token_response.success:
                        ...
                        if error_code == "10004":
                            logger.error(f"Non-retryable error: HTTP 200 with error_code 10004: {error_msg}")
                            raise last_error
                        
                        # For other errors, allow retry
                        if attempt < max_retries:
                            logger.warning(f"Authentication failed with error_code {error_code}, retrying... ({attempt + 1}/{max_retries})")
                            await asyncio.sleep(1)  # Wait 1 second before retry
                            continue
                        raise last_error
                    ...
                    if not token_response.data:
                        raise AuthenticationError("Token data is missing in successful response")
                ...
                last_error = AuthenticationError(f"Network request failed: {e}")
                if attempt < max_retries:
                ...
                last_error = AuthenticationError(f"Error occurred while getting token: {e}")
                if attempt < max_retries:
                    logger.warning(f"Unexpected error, retrying... ({attempt + 1}/{max_retries}): {e}")
                    await asyncio.sleep(1)  # Wait 1 second before retry
                    continue
                raise last_error
        ...
        if last_error:
            raise last_error
        raise AuthenticationError("Unknown error occurred during token acquisition")
