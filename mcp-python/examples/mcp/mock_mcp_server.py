...
class MockMCPServer:
    ...
    def _register_tools(self):
        ...
        def take_photo(
            ...
            is_view: Annotated[bool, Field(default=False, description="Whether to view the photo after taking it; e.g. 'true'")] = False
        ) -> str:
            """
            Take a photo
            """
            try:
                ...
                if system == "Darwin":  # macOS
                    try:
                        ...
                        result = subprocess.run(['imagesnap', file_path], capture_output=True, text=True)
                        if result.returncode == 0:
                            ...
                            return f"Error: imagesnap not found or failed. Install with: brew install imagesnap"
                    except FileNotFoundError:
                        return f"Error: imagesnap not found. Install with: brew install imagesnap"
                        
                elif system == "Windows":  # Windows
                    # For Windows, we'll use a placeholder approach
                    return f"Photo capture on Windows requires additional software. Consider using built-in Camera app."
                    
                elif system == "Linux":  # Linux
                    # Try fswebcam or other tools
                    tools = [
                        (['fswebcam', '-r', '640x480', '--no-banner', file_path], 'fswebcam'),
                        (['ffmpeg', '-f', 'v4l2', '-i', '/dev/video0', '-vframes', '1', file_path], 'ffmpeg'),
                    ]
                    ...
                    for cmd, tool_name in tools:
                        try:
                            ...
                            continue
                    return f"Error: No suitable camera tool found on Linux. Try installing fswebcam or ffmpeg."
                    
                else:
                    return f"Error: Unsupported operating system: {system}"
                    
            except Exception as e:
                logger.error(f"Take photo error: {e}")
        ...
        def view_photo(
            ...
            try:
                ...
                logger.error(f"View photo error: {e}")
        ...
        def stop_music() -> str:
            ...
            try:
                ...
                if system == "Darwin":  # macOS
                    ...
                    subprocess.run(['pkill', '-f', 'Music'], capture_output=True)
    ...
    async def start_server(self, host: str = "localhost", port: int = 8765):
        ...
        logger.info(f"🌐 Starting Mock MCP Server on http://{host}:{port}")
